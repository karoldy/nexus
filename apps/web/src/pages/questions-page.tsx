...
import { deleteQuestion, listQuestions } from '@/apis';
...
export function QuestionsPage() {
  ...
  const list = useQuery({
    queryKey: ['questions', page, q, published, type],
    queryFn: () =>
      listQuestions({
        page,
        pageSize: 10,
  ...
  return (
    <WorkspaceShell title={t('question.title')}>
      <div className="mb-4 flex flex-wrap items-end gap-3">
        <Field className="w-56">
          <FieldLabel htmlFor="q">{t('question.search')}</FieldLabel>
          <Input
            id="q"
            value={q}
            onChange={(event) => {
        ...
        <Field className="w-40">
          <FieldLabel htmlFor="published">{t('question.published')}</FieldLabel>
          <Select
            value={published}
            items={[
              { value: 'all', label: t('question.all') },
              { value: 'true', label: t('question.publishedYes') },
              { value: 'false', label: t('question.draft') },
            ]}
            onValueChange={(value) => {
          ...
          >
            ...
            <SelectContent>
              <SelectItem value="all">{t('question.all')}</SelectItem>
              <SelectItem value="true">{t('question.publishedYes')}</SelectItem>
              <SelectItem value="false">{t('question.draft')}</SelectItem>
            </SelectContent>
          </Select>
        </Field>
        <Field className="w-48">
          ...
          <Select
            ...
            onValueChange={(value) => {
              if (value !== 'all' && !QUESTION_TYPES.includes(value as QuestionType)) {
                return;
              }
              setPage(1);
              setType(value as 'all' | QuestionType);
            }}
          >
        ...
        {canCreate ? (
          <Link className={buttonVariants()} to={paths.questionNew}>
            {t('question.create')}
          </Link>
        ) : null}
      </div>
        ...
        {(list.data?.records ?? []).map((item) => (
          <Card key={item.id}>
            <CardContent className="flex flex-wrap items-center justify-between gap-3 pt-6">
              <div>
                <Link className="font-medium hover:underline" to={paths.questionEdit(item.id)}>
                  {item.stem}
                </Link>
                <p className="text-sm text-muted-foreground">
                  {t(`question.types.${item.type}`)}
                  {` · ${t('question.difficultyValue', { value: item.difficulty })}`}
                  {` · ${item.published ? t('question.publishedYes') : t('question.draft')}`}
